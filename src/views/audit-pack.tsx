...
import type { ActionResult, ViewProps } from './types'

/** The columns of an audit-pack row this screen renders. */
export type PackRow = {
  packId: string
  status: string
  scope: string
...
export function AuditPackView({
  ...
  packs: PackRow[]
  /** `PACK_TTL_DAYS`; passed in because the module that owns it is server-only. */
  ttlDays: number
    ...
    <div className="block">
      ...
      >
                ...
                <span style={{ textAlign: 'right' }}>
                  {p.status === 'ready' ? (
                    <a
                      className="mono"
                      style={{ fontSize: 11.5 }}
                      href={`${base}/audit-pack/${p.packId}/download`}
                    >
                      Download
                    </a>
                  ) : (
        ...
        >
          ...
          <RequestPack disabled={!canWrite} onRequest={onRequest} />
          <Link href={`${base}/evidence`} className="btn btn-ghost" style={{ textAlign: 'center' }}>
